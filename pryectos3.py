...
def get_default_config(type_scen):
    ...
    return {
        ...
        # Deuda Mezzanine y Relacionada
        "prestamo_relacionada": {
            "monto": 5000.0, 
            "tasa_anual": 8.0
        },
        "lista_kps": [
            {"nombre": "KP 1", "monto": 2000.0, "tasa_anual": 12.0, "plazo": 24}
        ],
        
        "otros_costos": [{"nombre": "Arquitectura", "valor": 2000.0}],
...
def calcular_flujo(data):
    ...
    v_otros = sum([item["valor"] for item in data["otros_costos"]])
    ...
    # --- INICIALIZACIÓN DEUDA KPs y RELACIONADA ---
    ...
    equity_otros = v_otros
    inversion_inicial = equity_terreno + equity_otros
    ...
    for m in range(1, horizonte + 1):
        ...
        # 1. MOVIMIENTO DEUDA BANCARIA
        egreso_equity_mes = 0
        if m <= duracion:
            costo_mes_total = v_cont / duracion
            giro_banco = costo_mes_total * pct_fin_const
            egreso_equity_mes = costo_mes_total - giro_banco 
            
            saldo_const_uf += giro_banco * pct_uf
            saldo_const_clp_nominal += (giro_banco * pct_clp) * factor_uf 

        int_uf = (saldo_const_uf + saldo_terr_uf) * tasa_mensual_uf
        saldo_const_uf += int_uf
        
        if m == 1: saldo_terr_clp_nominal *= 1.0 
        int_clp_nom = (saldo_const_clp_nominal + saldo_terr_clp_nominal) * tasa_mensual_clp
        saldo_const_clp_nominal += int_clp_nom
        
        interes_acum_banco_total += int_uf + (int_clp_nom / factor_uf)

        # 2. MOVIMIENTO DEUDA KPs
        saldo_total_kps = 0
        for kp in kps_activos:
            if kp["saldo"] > 0:
                int_kp = kp["saldo"] * kp["tasa_mensual"]
                kp["saldo"] += int_kp
                kp["interes_acumulado"] += int_kp
                interes_acum_kps += int_kp
            saldo_total_kps += kp["saldo"]

        # 3. MOVIMIENTO RELACIONADA
        if saldo_relacionada > 0:
            int_rel = saldo_relacionada * tasa_mensual_rel
            saldo_relacionada += int_rel
            interes_acum_relacionada += int_rel

        # 4. WATERFALL (PAGOS)
        ingreso_uf = sum([r["Monto"] for r in recuperos if r["Mes"] == m])
        dinero_disponible = ingreso_uf
        ...
        # A. Banco
        pago_banco = 0
        if dinero_disponible > 0 and deuda_banco_total > 0:
            pago_max_banco = min(deuda_banco_total, dinero_disponible)
            es_rango_terreno = (m >= inicio_pago_t and m <= fin_pago_t)
            p_terr, p_const = 0, 0
            
            if es_rango_terreno:
                if prioridad_t:
                    p_terr = min(real_terr_uf, pago_max_banco)
                    p_const = min(real_const_uf, pago_max_banco - p_terr)
                else:
                    if deuda_banco_total > 0:
                        p_terr = pago_max_banco * (real_terr_uf / deuda_banco_total)
                        p_const = pago_max_banco * (real_const_uf / deuda_banco_total)
            else:
                p_const = min(real_const_uf, pago_max_banco)
            
            if p_terr > 0 and real_terr_uf > 0:
                prop_uf = saldo_terr_uf / real_terr_uf
                prop_clp = (saldo_terr_clp_nominal/factor_uf) / real_terr_uf
                saldo_terr_uf -= (p_terr * prop_uf)
                saldo_terr_clp_nominal -= (p_terr * prop_clp * factor_uf)

            if p_const > 0 and real_const_uf > 0:
                prop_uf = saldo_const_uf / real_const_uf
                prop_clp = (saldo_const_clp_nominal/factor_uf) / real_const_uf
                saldo_const_uf -= (p_const * prop_uf)
                saldo_const_clp_nominal -= (p_const * prop_clp * factor_uf)
            
            pago_banco = p_terr + p_const
            dinero_disponible -= pago_banco
        
        # B. KPs
        pago_kps = 0
        if dinero_disponible > 0 and saldo_total_kps > 0:
            pago_max_kps = min(saldo_total_kps, dinero_disponible)
            for kp in kps_activos:
                if kp["saldo"] > 0 and saldo_total_kps > 0:
                    peso = kp["saldo"] / saldo_total_kps
                    abono = pago_max_kps * peso
                    kp["saldo"] -= abono
            pago_kps = pago_max_kps
            dinero_disponible -= pago_kps
            
        # C. Relacionada
        pago_rel = 0
        if dinero_disponible > 0 and saldo_relacionada > 0:
            pago_rel = min(saldo_relacionada, dinero_disponible)
            saldo_relacionada -= pago_rel
            dinero_disponible -= pago_rel

        flujo_neto_mes = dinero_disponible - egreso_equity_mes
    ...
    inversion_equity_total = inversion_inicial 
    
    utilidad = data["valor_venta_total"] - (v_terr + v_cont + v_otros + costo_fin_total)
    costo_proyecto_total = v_terr + v_cont + v_otros + costo_fin_total
...
with col_inputs:
    ...
    def render_scenario_inputs(scen_key):
        ...
        with st.container():
            with st.expander("🏗️ Proyecto Base", expanded=False):
            ...
            with st.expander("🤝 Deuda Privada (KPs y Relac.)", expanded=True):
                ...
                if st.button("➕ Agregar KP", key=f"add_kp_{scen_key}"):
                    data["lista_kps"].append({"nombre": f"KP {len(data['lista_kps'])+1}", "monto": 1000.0, "tasa_anual": 10.0, "plazo": 24})
                    st.rerun()
                ...
                for i, kp in enumerate(data["lista_kps"]):
                    ...
                    kp["plazo"] = k3.number_input("Plazo (Meses)", value=int(kp["plazo"]), key=f"kpp_{scen_key}_{i}")
...
with col_dash:
    ...
    # --- NUEVA TARJETA DE INTERESES (SOLICITADA) ---
    st.markdown("#### 💳Costos Financieros (Acumulado)")
    with st.container():
        # Hacemos un contenedor visualmente distinto
    ...
    with st.expander("📋 Tabla Detallada", expanded=False):
        cols_show = ["Mes", "Deuda Banco", "Deuda KPs", "Deuda Relac.", "Ingresos", "Inversión", "Flujo Neto", "Flujo Acumulado"]
