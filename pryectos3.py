...
def get_default_config(type_scen):
    ...
    return {
        ...
        "total_otros_costos_inicial": otros_costos_ini, # Inversión inicial (Permisos, etc)
        "otros_costos_mensuales": 100.0, # Gastos operativos recurrentes (Admin, Ventas)
...
def calcular_flujo(data):
    ...
    # El "v_otros_inicial" sale del bolsillo (Equity) al inicio
    ...
    total_otros_costos_operativos = 0 # Suma de los gastos mensuales
    ...
    for m in range(1, horizonte + 1):
        ...
        # Capitalizamos intereses Banco temporalmente (se intentarán pagar abajo)
        ...
        for kp in kps_activos:
            if kp["saldo"] > 0:
                ...
                kp["saldo"] += ik # Capitaliza
                int_kps_mes += ik
        interes_acum_kps += int_kps_mes
        
        # Relacionada
        int_rel_mes = 0
        if saldo_relacionada > 0:
            int_rel_mes = saldo_relacionada * tasa_mensual_rel
            saldo_relacionada += int_rel_mes # Capitaliza
        interes_acum_relacionada += int_rel_mes
        
        # Total intereses devengados este mes (para validación de pago)
        total_interes_devengado_mes = int_banco_mes_en_uf + int_kps_mes + int_rel_mes

        # 3. FLUJO OPERATIVO
        ingreso_uf = sum([r["Monto"] for r in recuperos if r["Mes"] == m])
        
        # RESTAR OTROS COSTOS OPERATIVOS (No financieros)
        # Si estamos dentro del horizonte de venta o construcción, cobramos el gasto mensual
        gasto_operativo_mes = v_otros_mensual if (m <= recepcion + 6) else 0 
        total_otros_costos_operativos += gasto_operativo_mes
        
        # DINERO DISPONIBLE PARA PAGAR DEUDA
        # (Ventas - Gastos Operativos). Si es negativo, no hay para pagar deuda.
        flujo_operativo = ingreso_uf - gasto_operativo_mes
        dinero_para_deuda = max(0.0, flujo_operativo)
        
        # 4. WATERFALL DE PAGOS (INTERÉS PRIMERO, CAPITAL DESPUÉS)
        ...
        if dinero_para_deuda > 0 and deuda_banco_total > 0:
            # 1. Definir cuánto pagamos al banco (Tope: Deuda Total o Dinero Disponible)
            monto_a_pagar_banco = min(deuda_banco_total, dinero_para_deuda)
            
            # 2. Desglosar ese pago en Interés vs Capital (Teórico para reporte)
            # Primero cubrimos el interés generado este mes por el banco
            pago_banco_interes = min(monto_a_pagar_banco, int_banco_mes_en_uf)
            pago_banco_capital = monto_a_pagar_banco - pago_banco_interes
            
            # 3. Aplicar reducción de saldos (Matemática de prorrata)
            # Lógica Terreno vs Construcción (Prioridad)
            ...
            # Ejecutar resta saldos (UF y CLP)
        ...
        if dinero_para_deuda > 0 and saldo_total_kps > 0:
            monto_a_pagar_kps = min(saldo_total_kps, dinero_para_deuda)
            
            # Prioridad Interés KP
            pago_kps_interes = min(monto_a_pagar_kps, int_kps_mes)
            
            # Prorrata KPs
            for kp in kps_activos:
                if kp["saldo"] > 0 and saldo_total_kps > 0:
                    peso = kp["saldo"] / saldo_total_kps
                    abono = monto_a_pagar_kps * peso
                    kp["saldo"] -= abono
            
            pago_kps_total = monto_a_pagar_kps
        ...
        # Flujo Neto Final = (Ingresos - Gastos Op) - (Pago Deuda Total) - (Aporte Equity Construcción)
        # Nota: (Ingresos - Gastos Op - Pago Deuda Total) es lo que sobró en 'dinero_para_deuda'
        
        flujo_neto_mes = dinero_para_deuda - egreso_equity_const 
        
        # Si el flujo operativo fue negativo (ingresos < gastos), el flujo neto parte negativo
    ...
    # Costo Total Proyecto = Terreno + Const + Otros(Ini) + Otros(Op) + Fin
...
with col_dash:
    ...
    with st.expander("📋 Tabla Detallada (Verificación de Pagos)", expanded=False):
        # Mostramos columnas clave para verificar que se paga interés y luego capital
